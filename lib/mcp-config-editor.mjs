...
export function renderEditor(state, { configPath, version } = {}) {
  ...
  // La leyenda va aquí, en el módulo con tests, y no en la cáscara.
  //
  // Existía una en `bin/oxidegate-mcp.mjs` desde el primer commit del
  // selector, así que la premisa original de #25 —"no hay leyenda"— era
  // FALSA: se buscó solo en lib/ y se concluyó sobre todo el repo. Lo que sí
  // fallaba: decía `d interruptor`, sin nombrar QUÉ apaga ni qué pasa al
  // apagarlo, y un usuario desactivó la función entera creyendo que apagaba
  // un servidor.
  //
  // Dos consecuencias, y las dos importan. Nombrar el efecto: `d válvula`,
  // no `d interruptor`. Y vivir DONDE HAY TESTS: en la cáscara nadie podía
  // asegurar que se pintara, ni que dijera la verdad — es el mismo error que
  // ya se cometió con el pie, que vivía en printOnce y el modo interactivo
  // nunca lo enseñaba.
  lines.push('');
