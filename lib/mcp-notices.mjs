 ...
 */
export function startupNotice({ partition, plan }) {
  ...
  if (partition.notConnected.length > 0) {
    parts.push(
      `empiezas con ${partition.notConnected.length} MCP sin conectar: ${list(partition.notConnected)}.` +
        (partition.connected.length > 0 ? ` Siguen activos: ${list(partition.connected)}.` : ''),
    );
  }
