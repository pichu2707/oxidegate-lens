...
/**
 * Lo que declara GET /version — el contrato de capacidades que la lens
 * empezó a leer con la issue #30. Observación pasada ENTERA, igual que
 * `snapshot` en `checkPrice`: es un resultado de `lib/proxy-version.mjs`
 * en su forma `{status, ...}`, no campos sueltos.
 *
 * Un 404 no cuenta como fallo aquí: `lib/proxy-version.mjs` lo modela como
 * `{ status: 'known', reason: 'pre-contract' }` — el proxy SÍ contestó algo
 * útil, solo que es anterior a tener /version. Eso vale un aviso, no un
 * unknown: la comprobación se pudo hacer y trajo una respuesta.
 *
 * Pero esa lectura solo vale si quien contestó ES OxideGate. `checkHealth` y
 * `checkTraffic` ya comprueban `isOxidegate` antes de interpretar su
 * observación; esta comprobación se quedó atrás y lo pagó en producción: un
 * intruso en el puerto que 404 en /version (el comportamiento por defecto de
 * casi cualquier servidor) generaba «tu OxideGate es anterior a /version» —
 * una versión de OxideGate afirmada sobre un servicio que no es OxideGate.
 * Por eso primero se comprueba identidad, igual que las vecinas.
 */
function checkVersion({ reachable, isOxidegate, version }) {
  if (!reachable) {
    return {
      id: 'version',
      status: 'unknown',
      title: 'No se pudo sondear /version',
      detail: 'La comprobación no llegó a hacerse.',
      action: 'Resuelve primero la conectividad y vuelve a ejecutar.',
    };
  }
  if (!isOxidegate) {
    return {
      id: 'version',
      status: 'unknown',
      title: 'No se puede interpretar /version',
      detail: 'El puerto contestó algo, pero lo que hay ahí no es OxideGate: su contrato de capacidades no es fiable.',
      action: 'Apunta al puerto correcto (OXIDEGATE_PORT) antes de leer nada de esta ruta.',
    };
  }
  if (version?.status === 'known' && version.reason === 'pre-contract') {
    return {
      id: 'version',
      status: 'warn',
      title: 'Tu OxideGate es anterior a /version',
      detail: 'Esta versión del proxy no publica su contrato de capacidades: es anterior a la introducción de /version.',
      action:
        'Actualiza OxideGate (brew upgrade oxidegate) para que la lens sepa qué endpoints y ' +
        'campos publica sin tener que adivinarlo sondeando por ausencia.',
    };
  }
  if (version?.status === 'known') {
    const nEndpoints = version.endpoints?.length ?? 0;
    const nFields = version.fields?.length ?? 0;
    return {
      id: 'version',
      status: 'ok',
      title: `OxideGate ${version.oxidegate} publica el contrato ${version.contract}`,
      detail: `Declara ${nEndpoints} endpoint(s) y ${nFields} campo(s).`,
      action: '',
    };
  }
  return {
    id: 'version',
    status: 'unknown',
    title: 'No se pudo leer /version',
    detail: `Razón: ${version?.reason ?? 'desconocida'}.`,
    action: 'Resuelve primero la conectividad y vuelve a ejecutar.',
  };
}
 ...
 */
/**
 * La copia que OpenCode se guarda del plugin.
 *
 * Existe porque 0.6.1 se publicó con el arreglo y el instalador seguía
 * fallando idéntico a 0.6.0: leía una copia cacheada en un directorio
 * llamado `@latest` que tenía la versión CLAVADA. El mensaje de error
 * describía con toda precisión un paquete que ya no era el publicado —
 * un dato correcto sobre el objeto equivocado, que es la forma más cara
 * de equivocarse.
 *
 * Ausente es `ok` y no `unknown` a propósito: quien instala por Homebrew y
 * cablea por ruta NUNCA tendrá esta caché, y dejarle el diagnóstico en
 * "desconocido" para siempre sería ruido, no honestidad. Lo que no se
 * pudo LEER sí es unknown.
 */
function checkOpenCodeCache({ opencodeCache }) {
  const id = 'opencode-cache';
  const estado = opencodeCache?.status;

  if (estado === 'match') {
    return { id, status: 'ok', title: 'La copia que usa OpenCode está al día', detail: `Versión ${opencodeCache.cached}.`, action: '' };
  }
  if (estado === 'absent') {
    return {
      id,
      status: 'ok',
      title: 'OpenCode no tiene copia en caché del plugin',
      detail: 'No hay nada que pueda quedarse viejo.',
      action: '',
    };
  }
  if (estado === 'mismatch') {
    return {
      id,
      status: 'warn',
      title: 'OpenCode está usando otra versión del plugin',
      detail: `En su caché tiene la ${opencodeCache.cached} y tú ejecutas la ${opencodeCache.running}.`,
      action:
        'El directorio se llama @latest pero la versión de dentro está clavada y no vuelve a ' +
        'resolver: publicar una versión nueva no lo cambia. Límpialo con ' +
        '`oxidegate-savings --clear-opencode-cache` y reinstala el plugin.',
    };
  }
  return {
    id,
    status: 'unknown',
    title: 'No se pudo saber qué versión tiene OpenCode en caché',
    detail: 'La copia está pero no se pudo leer.',
    action: 'Si el instalador de OpenCode se comporta raro, límpiala con `oxidegate-savings --clear-opencode-cache`.',
  };
}
...
export function diagnose(observations) {
  const checks = [
    ...
    checkOpenCodeCache(observations),
  ];
