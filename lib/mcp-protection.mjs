 ...
 */
export function readProtectedServers({ path = resolveProtectionConfigPath(), env = process.env, projectConfig } = {}) {
  ...
  // Capa de PROYECTO, entre la variable y el fichero global. Sólo cuenta si
  // está APROBADA: un `.oxidegate-lens.json` de un repo clonado es código
  // ajeno (ver lib/mcp-project-config.mjs). Y REEMPLAZA a lo global en vez de
  // fusionarse — decisión de seguridad: se lee un fichero y se sabe la
  // respuesta, en lugar de componer dos mentalmente.
  //
  // Callar sobre la clave NO es declararla vacía, igual que en el global: un
  // proyecto que sólo toca el interruptor no borra la lista de nadie.
  if (projectConfig?.status === 'approved' && Array.isArray(projectConfig.config?.[CONFIG_KEY])) {
    const declared = projectConfig.config[CONFIG_KEY];
    if (!declared.every((entry) => typeof entry === 'string')) {
      return { status: 'unknown', reason: 'unrecognized-shape', source: 'project' };
    }
    return { status: 'known', servers: declared.map((e) => e.trim()).filter(Boolean), source: 'project' };
  }
 ...
 */
export function readDisableByDefault({ path = resolveProtectionConfigPath(), env = process.env, projectConfig } = {}) {
  ...
  if (projectConfig?.status === 'approved' && SWITCH_KEY in (projectConfig.config ?? {})) {
    const value = projectConfig.config[SWITCH_KEY];
    if (typeof value !== 'boolean') {
      return { status: 'unknown', reason: 'unrecognized-shape', source: 'project' };
    }
    return { status: 'known', enabled: value, source: 'project' };
  }
