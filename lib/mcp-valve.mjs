...
const REASON = {
  ...
  TOOLS_FLATTENED: 'tools-flattened',
 ...
 */
function recommendationFor(row, usage, joinHealth, attributionUsable) {
  // FIRST, because it outranks every other refusal below — including the
  // window gate. `tools-flattened` and `insufficient-observation` imply
  // OPPOSITE actions: one is waited out, the other never is. On a route that
  // flattens, more traffic can never produce per-server attribution, so
  // reporting the transient reason sends the user to wait forever. It also
  // blocks a FULL window: a zero measured while tools were flattened is not
  // a zero, it is a server that may have been used inside a request that
  // could not name it.
  // Sigue siendo el primer rechazo, y por el mismo motivo: en una ruta que
  // aplana, más tráfico NUNCA produce atribución por servidor, así que dar la
  // razón transitoria mandaría a esperar para siempre.
  //
  // La excepción no es "esperar más": es haber resuelto el cruce por nombres
  // contra la lista autoritativa del snapshot (`attributeFlattenedNames`).
  // Cuando eso ocurre la atribución existe de verdad, y seguir rechazando
  // aquí escondería una respuesta que YA se tiene.
  if (usage.hasFlattenedTools === true && attributionUsable !== true) {
    return { status: 'no-recommendation', reason: REASON.TOOLS_FLATTENED };
  }
...
/**
 * Resuelve la atribución de un cable APLANADO cruzando los nombres observados
 * (`usage.usesByToolName`) contra la lista autoritativa de tools por servidor
 * del snapshot (`server.toolNames`).
 *
 * NO es una heurística de nombres — eso es lo que OxideGate#5 pidió y su PR#9
 * rechazó con razón: partir `delegation_list` por `_` haría de `delegation`
 * un servidor MCP inexistente. Aquí un nombre solo se atribuye si aparece en
 * la lista que un servidor DECLARA, por una de dos formas medidas en tráfico
 * real: cruda (`pi` manda `mem_search`) o prefijada por el servidor
 * (`opencode` manda `engram_mem_search`).
 *
 * Devuelve `{ usable: false }` —y el bloqueo por `tools-flattened` se
 * mantiene— en cuanto algo impide una atribución COMPLETA:
 *
 *   - la ventana no está aplanada (no hace falta esta vía),
 *   - alguna fila aplanada no trajo nombres (`flattenedNamesComplete`),
 *   - ningún servidor del snapshot declara sus tools,
 *   - o un nombre casa con MÁS DE UN servidor.
 *
 * El último es el importante: atribuirlo a uno sería inventar, y atribuirlo a
 * ninguno dejaría una cuenta parcial que se lee como completa. Una atribución
 * a medias es peor que ninguna, porque un cero parcial parece un cero medido.
 */
function attributeFlattenedNames({ snapshotServers, usage }) {
  if (usage.status !== 'observed') return { usable: false };
  if (usage.hasFlattenedTools !== true) return { usable: false };
  if (usage.flattenedNamesComplete !== true) return { usable: false };

  const conListas = snapshotServers.filter((s) => Array.isArray(s.toolNames));
  if (conListas.length === 0) return { usable: false };

  // nombre observable -> servidores que lo declaran. Se indexan las dos formas
  // medidas; si dos servidores producen la misma, el nombre es ambiguo.
  const porNombre = new Map();
  const indexar = (clave, servidor) => {
    if (!porNombre.has(clave)) porNombre.set(clave, new Set());
    porNombre.get(clave).add(servidor);
  };
  for (const server of conListas) {
    for (const tool of server.toolNames) {
      indexar(tool, server.name);
      indexar(`${server.name}_${tool}`, server.name);
      indexar(`${sanitizeServerName(server.name)}_${tool}`, server.name);
    }
  }

  const usesByLabel = {};
  for (const [nombre, veces] of Object.entries(usage.usesByToolName ?? {})) {
    const duenos = porNombre.get(nombre);
    if (!duenos) continue; // no casa con ninguna lista MCP: es nativa de verdad
    if (duenos.size > 1) return { usable: false }; // ambiguo: no se inventa
    const dueno = [...duenos][0];
    usesByLabel[dueno] = Math.max(usesByLabel[dueno] ?? 0, veces);
  }

  // Todo servidor con lista declarada entra con su cuenta, aunque sea 0: ese
  // cero YA es un cero medido, que es justo lo que esta vía desbloquea.
  for (const server of conListas) {
    usesByLabel[server.name] = usesByLabel[server.name] ?? 0;
  }

  return { usable: true, usesByLabel };
}

export function buildValveRows({ snapshot, usage }) {
  const snapshotServers = snapshot.status === 'known' ? snapshot.servers : [];
  const flattenedJoin = attributeFlattenedNames({ snapshotServers, usage });
  const usesByLabel =
    usage.status === 'observed'
      ? { ...usage.usesByLabel, ...(flattenedJoin.usable ? flattenedJoin.usesByLabel : {}) }
      : {};
  const wireLabelSet = usage.status === 'observed' ? new Set(Object.keys(usesByLabel)) : new Set();
  ...
  // La cuenta de usos sólo es utilizable si hubo observación suficiente Y el
  // cable conservó la atribución por servidor. Con `tools_flattened` un `0`
  // deja de significar "no se usó" y pasa a significar "no se pudo saber": el
  // servidor pudo usarse dentro del bloque aplanado. Bloquear la
  // recomendación no basta — la fila NO puede imprimir ese cero, o la
  // invariante de honestidad se rompe en el render aunque la decisión fuera
  // correcta. Pinchado por un test a nivel CLI, que es donde se vio.
  // Con el cable aplanado el `0` dejaba de significar "no se usó" y pasaba a
  // "no se pudo saber". Sigue siendo así MIENTRAS no se pueda resolver el
  // cruce por nombres: si se resuelve (`flattenedJoin.usable`), la atribución
  // vuelve a ser utilizable y el cero vuelve a ser un cero medido.
  const attributionUsable =
    usage.status === 'observed' && (usage.hasFlattenedTools !== true || flattenedJoin.usable);
  ...
  for (const [sanitizedKey, group] of bySanitized) {
    ...
    if (match) {
      ...
      rows.push({
        ...
        uses: attributionUsable ? usesByLabel[match.label] : undefined,
        recommendation: undefined,
      });
      continue;
    }
    ...
    rows.push({
      ...
      enabled: server.enabled,
      tokens: server.tokens,
      uses: attributionUsable ? 0 : undefined,
      recommendation: undefined,
    });
  }
  ...
  if (usage.status === 'observed') {
    for (const label of Object.keys(usesByLabel)) {
  ...
  for (const row of rows) {
    row.recommendation = recommendationFor(row, usage, joinHealth, attributionUsable);
  }
