...
// `fetchImpl` with a real default (`fetch`) — the same INJECTION PATTERN as
// `verify` in `lib/mcp-endpoint.mjs` (the binary owns fetch, the module
// receives it), not the same function shape: `verify` has no default and
// returns `{reachable, isOxidegate}`, not a fetch response. NEVER throws.
//
// Exports:
