...
//     toolNames: string[] | undefined,  // RAW tool names this server declares.
//                                        // Same honesty invariant as bytes and
//                                        // tokens: ABSENT stays `undefined`,
//                                        // never coerced to `[]`. See below.
//   }
//
// WHY `toolNames` EXISTS, AND WHY ABSENT MUST NOT BECOME `[]`
// ------------------------------------------------------------
// It is the authoritative side of the join that lets `lib/mcp-valve.mjs`
// attribute spend when the wire arrives FLATTENED — when the client sends
// every tool in one undifferentiated bucket and OxideGate can only publish
// the raw names it saw (`tools_by_server[].tool_names`). The proxy cannot
// know which server a flattened name belongs to; this list can tell it.
//
// `undefined` means "this snapshot says nothing about this server's tools" and
// must BLOCK attribution. `[]` would mean "this server declares no tools at
// all" — a stronger, different claim that would let the join conclude a name
// is NOT this server's. Collapsing the two would manufacture exactly the kind
// of confident wrong answer this codebase refuses everywhere else.
//
...
/**
 * RAW tool names declared by a snapshot entry, or `undefined` when the entry
 * carries no usable `tools` array.
 *
 * Entries without a string `name` are dropped rather than smuggled through as
 * `undefined` names: a malformed row must not become a name that could match
 * nothing and be mistaken for evidence. A present-but-empty array is preserved
 * as `[]` — see the honesty note in the header for why that differs from
 * absent.
 */
function toolNamesForEntry(entry) {
...
export function readMcpSavingsSnapshot({ path = resolveSnapshotPath(), now = Date.now() } = {}) {
  ...
  for (const entry of measurements) {
    ...
    servers.push({
      ...
      toolNames: toolNamesForEntry(entry),
    });
  }
